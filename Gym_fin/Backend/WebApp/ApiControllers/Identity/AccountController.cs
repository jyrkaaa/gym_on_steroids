...
public class AccountController : ControllerBase
{
    ...
    private readonly UserManager<App.Domain.EF.Identity.AppUser> _userManager;
    private readonly ILogger<AccountController> _logger;
    private readonly SignInManager<App.Domain.EF.Identity.AppUser> _signInManager;
    ...
    /// Constructor
    /// </summary>
    public AccountController(IConfiguration configuration, UserManager<App.Domain.EF.Identity.AppUser> userManager,
        SignInManager<App.Domain.EF.Identity.AppUser> signInManager, ILogger<AccountController> logger, AppDbContext context)
    {
    ...
    )
    {
        ...
        if (appUser == null)
        {
            ...
            return NotFound(UserPassProblem);
        }
    ...
    )
    {
        ...
        appUser = new App.Domain.EF.Identity.AppUser()
        {
    ...
    )
    {
        ...
        {
            return BadRequest("Cant parse the token");
        }
    ...
    public async Task<ActionResult> Logout([FromBody] LogoutInfo logout)
    {
        ...
        await _context.Entry(appUser)
            ...
            .Query()
            .Where(x =>
                (x.UserId == User.GetUserId()) || // KEEPS IN DB after expiring (x.RefreshToken == logout.RefreshToken) ||
