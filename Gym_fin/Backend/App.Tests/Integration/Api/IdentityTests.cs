...
public class IdentityTests: IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    public async Task JWT_Custom_Expiration()
    {
        ...
        var getResponseAuthExpired = await _client.GetAsync("/api/v1/Exercise/all");
    ...
    public async Task JWT_Refresh()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("/api/v1/account/login?jwtExpiresInSeconds=4", loginData);
        ...
        var oldJwtJson = new RefreshTokenModel() {
            Jwt = responseData.JWT,
            RefreshToken = responseData.RefreshToken
        };
        // Refresh JWT
        var refreshResponse = await _client.PostAsJsonAsync("/api/v1/account/RenewRefreshToken", oldJwtJson);
        
        var refreshedResponseData = await refreshResponse.Content.ReadFromJsonAsync<JWTResponse>();
        Assert.NotNull(refreshedResponseData?.JWT);
        ...
        var getResponse2 = await _client.GetAsync("/api/v1/Exercise/all");
